    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (!error) {
          ...
          Swal.fire("ลบเรียบร้อย!", "ข้อมูลถูกลบออกจากระบบแล้ว", "success");
        } else {
  ...
  const processedCafes = [...cafes]
    .filter((c) => {
      ...
      const matchesSearch =
        ...
        (c.location_text &&
          c.location_text.toLowerCase().includes(lowerTerm)) ||
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              จัดการ คาเฟ่/สถานที่
            </h1>
            <p className="text-gray-500 text-sm mt-1">
              ร้านค้าและสถานที่จัดงานทั้งหมด ({allCount} แห่ง)
            </p>
          </div>
          <button
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex bg-gray-100 p-1 rounded-lg self-start md:self-auto">
            ...
            >
              <span className="w-2 h-2 rounded-full bg-gray-400"></span> แบบร่าง
              ({draftCount})
            </button>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200 text-gray-500 text-sm">
                  ...
                  <th className="p-4 font-bold w-[180px] text-center">
                    จัดการ
                  </th>
                </tr>
              </thead>
                    ...
                    <tr key={item.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() =>
                              window.open(`/cafe/${item.id}`, "_blank")
                            }
                          ...
                          <button
                            onClick={() =>
                              navigate(`/admin/edit-cafe/${item.id}`)
                            }
