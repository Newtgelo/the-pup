...
// Import Icons
import {
  IconShare, IconX, IconMaximize, IconCalendar, IconMapPin, IconChevronRight,
  IconTicket, IconZoomIn, IconClock, IconPhone, IconUsers, IconLayout,
  IconBriefcase, IconChevronLeft,
} from "./icons/Icons";
...
export const EventDetail = ({ onTriggerToast }) => {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      ...
      if (error) setEvent(null); else setEvent(data);
      setLoading(false);
    };
    fetchEvent();
  }, [id]);
  ...
  const handleMapClick = () => { if (event.lat && event.lng) window.open(`https://www.google.com/maps/search/?api=1&query=${event.lat},${event.lng}`, "_blank"); else onTriggerToast("กำลังเปิดแผนที่..."); };
  const addToCalendar = () => { if (!event.start_date) { onTriggerToast("ไม่พบข้อมูลวันเวลา"); return; } const dateStr = event.start_date.replace(/-/g, ""); const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(event.title)}&dates=${dateStr}/${dateStr}&details=${encodeURIComponent(event.description)}&location=${encodeURIComponent(event.location_name)}`; window.open(url, "_blank"); onTriggerToast("เปิด Google Calendar แล้ว"); };
  const handleShare = async () => { const shareData = { title: event.title, url: window.location.href }; try { if (navigator.share) await navigator.share(shareData); else { await navigator.clipboard.writeText(shareData.url); onTriggerToast("คัดลอกลิงก์แล้ว"); } } catch (err) { console.log("Error:", err); } };
          ...
          <div className="hidden md:flex bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 min-h-[550px]">
            ...
            <div className="flex-1 p-8 lg:p-10 flex flex-col justify-center relative">
              ...
              <div className="space-y-6 my-6">
                <div className="flex items-start gap-4 group"><div className="w-12 h-12 rounded-full bg-orange-50 flex items-center justify-center text-[#FF6B00] shadow-sm border border-orange-100 group-hover:scale-110 transition flex-shrink-0"><IconCalendar size={24} /></div><div><p className="text-gray-900 font-bold text-lg">{event.date_display}</p><p className="text-gray-500 text-sm">{event.time_display || "วันงานแสดง"}</p></div></div>
                ...
                <div className="flex items-start gap-4 group"><div className="w-12 h-12 rounded-full bg-green-50 flex items-center justify-center text-green-600 shadow-sm border border-green-100 flex-shrink-0"><IconTicket size={24} /></div><div><p className="text-gray-900 font-bold text-xl">{event.ticket_price || "Coming Soon"}</p><p className="text-gray-500 text-sm">ราคาบัตร</p></div></div>
              </div>
              <div className="flex gap-3 mt-auto pt-8 border-t border-dashed border-gray-200">
                <a href={event.ticket_link || "#"} target={event.ticket_link ? "_blank" : "_self"} rel="noopener noreferrer" onClick={(e) => { if (!event.ticket_link) { e.preventDefault(); onTriggerToast("ยังไม่มีลิงก์จองบัตร"); } }} className="flex-1 bg-[#FF6B00] hover:bg-[#E65000] text-white py-4 px-6 rounded-xl font-bold text-lg transition shadow-lg hover:shadow-xl flex justify-center items-center gap-2 active:scale-95"><IconTicket size={24} /> จองบัตรเลย</a>
            ...
            {event.tags && (
                <div className="flex flex-wrap gap-2 mt-8 border-t border-gray-100 pt-6">
                    {event.tags.split(',').map((tag, index) => (
                        <span key={index} className="px-3 py-1 rounded-full bg-gray-50 border border-gray-200 text-gray-600 text-sm font-medium hover:bg-[#FF6B00] hover:text-white hover:border-[#FF6B00] transition cursor-pointer" onClick={() => navigate(`/search?q=${tag.trim()}`)}>#{tag.trim()}</span>
                    ))}
                </div>
            )}
          </div>
...
export const CafeDetail = ({ onTriggerToast }) => {
    ...
    useEffect(() => {
        const fetchCafe = async () => {
            ...
            if (error) { setCafe(null); } else {
                let processedImages = [];
                if (Array.isArray(data.images)) { processedImages = data.images; } 
                else if (typeof data.images === 'string' && data.images.startsWith('{')) { processedImages = data.images.replace(/^{|}$/g, '').split(',').map(url => url.replace(/"/g, '').trim()); }
    ...
    const handleNext = (e) => { e?.stopPropagation(); if (allImages.length <= 1) return; setSelectedImageIndex((prev) => (prev + 1) % allImages.length); };
    const handlePrev = (e) => { e?.stopPropagation(); if (allImages.length <= 1) return; setSelectedImageIndex((prev) => (prev === 0 ? allImages.length - 1 : prev - 1)); };
    const onTouchStart = (e) => { setTouchEnd(null); setTouchStart(e.targetTouches[0].clientX); };
    const onTouchMove = (e) => setTouchEnd(e.targetTouches[0].clientX);
    const onTouchEnd = () => { if (!touchStart || !touchEnd) return; const distance = touchStart - touchEnd; if (distance > 50) handleNext(); if (distance < -50) handlePrev(); };

    useEffect(() => { const handleKeyDown = (e) => { if (!isLightboxOpen) return; if (e.key === 'ArrowRight') handleNext(); if (e.key === 'ArrowLeft') handlePrev(); if (e.key === 'Escape') setIsLightboxOpen(false); }; window.addEventListener('keydown', handleKeyDown); return () => window.removeEventListener('keydown', handleKeyDown); }, [isLightboxOpen, allImages.length]);
    useEffect(() => { const handleScroll = () => { if (window.scrollY > 300) setShowStickyTabs(true); else setShowStickyTabs(false); }; window.addEventListener('scroll', handleScroll); return () => window.removeEventListener('scroll', handleScroll); }, []);

    const getFacilities = () => { if (!cafe || !cafe.facilities) return []; if (Array.isArray(cafe.facilities)) return cafe.facilities; if (typeof cafe.facilities === 'string') return cafe.facilities.split(','); return []; };
    ...
    const handleMap = () => { if (cafe.lat && cafe.lng) window.open(`https://www.google.com/maps/search/?api=1&query=${cafe.lat},${cafe.lng}`, '_blank'); else onTriggerToast("กำลังเปิดแผนที่..."); };
    const handleCall = () => { if (cafe.phone) window.location.href = `tel:${cafe.phone}`; else onTriggerToast("ไม่มีเบอร์โทรศัพท์"); };
    const handleShare = async () => { const shareData = { title: cafe.name, url: window.location.href }; try { if (navigator.share) await navigator.share(shareData); else { await navigator.clipboard.writeText(shareData.url); onTriggerToast("คัดลอกลิงก์แล้ว"); } } catch (err) { console.log("Error:", err); } };
      ...
      <>
        ...
        {isLightboxOpen && (
            <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center backdrop-blur-sm" onClick={() => setIsLightboxOpen(false)}>
                ...
                <div className="relative w-full max-w-5xl h-full flex items-center justify-center px-2 md:px-16" onClick={(e) => e.stopPropagation()} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}><SafeImage src={currentImage} alt="Full View" className="max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl select-none" /></div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-24 relative animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10 mt-8 md:mt-0">
                    ...
                    {allImages.length > 1 && (
                        <div className="flex overflow-x-auto gap-2 pb-2 md:grid md:grid-cols-6 md:gap-2 md:pb-0 scrollbar-hide snap-x">
                            {allImages.map((img, idx) => (
                                <div key={idx} onClick={() => setSelectedImageIndex(idx)} className={`flex-shrink-0 snap-start w-24 aspect-square md:w-auto rounded-xl overflow-hidden cursor-pointer border-2 transition-all ${selectedImageIndex === idx ? 'border-[#FF6B00] scale-95 ring-2 ring-[#FF6B00]/30' : 'border-transparent hover:border-gray-300'}`}>
                ...
                <div className="flex flex-col">
                    <div className="mb-6"><span className="bg-[#FF69B4] text-white text-xs font-bold px-3 py-1.5 rounded-full mb-3 inline-block">K-Pop Cafe</span><h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">{cafe.name}</h1><div className="flex items-center text-gray-500 text-sm"><IconMapPin size={16} className="mr-1"/> {cafe.location_text || "ไม่ระบุสถานที่"}</div></div>
                    <div className="flex bg-gray-100 p-1 rounded-xl mb-6"><button onClick={() => setActiveTab('general')} className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-bold transition-all ${activeTab === 'general' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>สำหรับลูกค้า</button><button onClick={() => setActiveTab('venue')} className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'venue' ? 'bg-white text-[#FF6B00] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}><IconBriefcase size={16} /> สำหรับผู้จัด</button></div>
                    ...
                    {activeTab === 'general' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="hidden md:flex gap-3"><button onClick={handleMap} className="flex-1 bg-[#FF6B00] hover:bg-[#E65000] text-white py-3 rounded-xl font-bold transition flex justify-center items-center gap-2 shadow-md active:scale-95"><IconMapPin size={18} /> ดูแผนที่</button><button onClick={handleCall} className="flex-1 border border-gray-300 hover:bg-gray-50 text-gray-700 py-3 rounded-xl font-medium transition flex justify-center items-center gap-2 active:scale-95"><IconPhone size={18} /> โทร</button></div>
                        </div>
                    )}
                    {activeTab === 'venue' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                             <div className="bg-orange-50 p-6 rounded-2xl border border-orange-100 space-y-5">
                                ...
                                <div className="grid grid-cols-2 gap-4"><div className="bg-white p-3 rounded-xl border border-orange-100"><div className="text-xs text-gray-500 mb-1 flex items-center gap-1"><IconUsers size={12}/> ความจุ</div><div className="font-bold text-gray-900">สอบถามร้าน</div></div><div className="bg-white p-3 rounded-xl border border-orange-100"><div className="text-xs text-gray-500 mb-1 flex items-center gap-1"><IconLayout size={12}/> พื้นที่</div><div className="font-bold text-gray-900">Indoor / Outdoor</div></div></div>
                                <div><p className="font-bold text-sm mb-2 text-gray-700">สิ่งอำนวยความสะดวก</p><div className="flex flex-wrap gap-2">{facilities.length > 0 ? facilities.map((fac, i) => (<span key={i} className="text-xs bg-white border border-orange-100 px-3 py-1.5 rounded-full text-gray-600">{fac.trim()}</span>)) : ( <span className="text-xs text-gray-400">- ไม่มีข้อมูล -</span> )}</div></div>
                            </div>
                            <div className="hidden md:flex gap-3"><button onClick={handleMap} className="flex-1 border border-gray-300 hover:bg-gray-50 text-gray-700 py-3 rounded-xl font-medium transition flex justify-center items-center gap-2 active:scale-95"><IconMapPin size={18} /> ดูแผนที่</button><button onClick={handleBooking} className="flex-1 bg-[#1E293B] hover:bg-black text-white py-3 rounded-xl font-bold transition flex justify-center items-center gap-2 shadow-md active:scale-95"><IconBriefcase size={18} /> สนใจจัดงานที่นี่</button></div>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="mt-16 pt-10 border-t border-gray-200">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {otherCafes.map((c) => {
                        ...
                        if (Array.isArray(c.images) && c.images.length > 0) { cImg = c.images[0]; } 
                        else if (typeof c.images === 'string' && c.images.startsWith('{')) { const parsed = c.images.replace(/^{|}$/g, '').split(',').map(s=>s.replace(/"/g, '')); if (parsed.length > 0 && parsed[0] !== "") cImg = parsed[0]; }
                        return (
                            <div key={c.id} onClick={() => navigate(`/cafe/${c.id}`)} className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg transition cursor-pointer group">
                                <div className="h-32 md:h-40 overflow-hidden bg-gray-100"><SafeImage src={cImg} alt={c.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500"/></div>
                                <div className="p-3"><h4 className="font-bold text-sm text-gray-900 line-clamp-1 group-hover:text-[#FF6B00] transition">{c.name}</h4><div className="mt-2 flex items-center gap-1 text-[10px] text-gray-400"><IconMapPin size={10}/> {(c.location_text || "").split(',')[0]}</div></div>
                            </div>
                        );
                    })}
                </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 px-4 md:hidden z-50 flex items-center gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] safe-area-bottom">
            {activeTab === 'general' ? (
                <><button onClick={handleMap} className="flex-1 bg-[#FF6B00] text-white h-12 rounded-xl font-bold text-base flex items-center justify-center gap-2 shadow-lg active:scale-95 transition"><IconMapPin size={20} /> ดูแผนที่</button><button onClick={handleCall} className="flex-1 border border-gray-200 text-gray-700 bg-white h-12 rounded-xl font-bold text-base flex items-center justify-center gap-2 hover:bg-gray-50 active:scale-95 transition"><IconPhone size={20} /> โทร</button></>
            ) : (
                <><button onClick={handleMap} className="flex-1 border border-gray-200 text-gray-700 bg-white h-12 rounded-xl font-bold text-base flex items-center justify-center gap-2 hover:bg-gray-50 active:scale-95 transition"><IconMapPin size={20} /> ดูแผนที่</button><button onClick={handleBooking} className="flex-[2] bg-[#1E293B] text-white h-12 rounded-xl font-bold text-base flex items-center justify-center gap-2 shadow-lg active:scale-95 transition"><IconBriefcase size={20} /> สนใจจัดงาน</button></>
            )}
        </div>
      </>
    );
