  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (error) {
        ...
        // 🔥 เช็คข้อมูลตรงนี้ (กด F12 ดูว่ามี tags ไหม)
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 relative animate-fade-in">
        ...
        <div className="mb-8 mt-12 md:mt-0">
          <div className="flex flex-wrap items-center gap-3 text-sm mb-4">
            <span className="bg-[#FF6B00] text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-sm">
              {news.category}
            </span>
        ...
        <div className="max-w-3xl mx-auto mb-10 text-lg text-gray-700 leading-relaxed font-serif">
          <ReactMarkdown
            components={{
              h2: ({ node, ...props }) => (
                <h2
                  className="text-2xl font-bold text-gray-900 mt-8 mb-4 border-l-4 border-[#FF6B00] pl-4"
                  {...props}
                />
              ),
              p: ({ node, ...props }) => (
              ...
              img: ({ node, ...props }) => (
                <div className="my-8">
                  <img className="w-full rounded-xl shadow-md" {...props} />
                  {props.title && (
        ...
        {/* Related Events (Hidden as requested) */}
...
export const EventDetail = ({ onTriggerToast }) => {
  ...
  const addToCalendar = () => {
    ...
    const dateStr = event.start_date.replace(/-/g, "");
  ...
  return (
    ...
    <p className="text-gray-900 font-bold text-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-10">
          ...
          {/* 🔥🔥 แทรกส่วน TAGS ตรงนี้ครับ (ต่อจาก Description) 🔥🔥 */}
                {event.tags && (
