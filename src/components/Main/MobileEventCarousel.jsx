                                ...
                                <div className={`bg-white rounded-xl shadow-lg overflow-hidden border transition-all duration-300 h-[140px] flex ${hoveredEventId === item.id ? 'border-gray-100 shadow-xl' : 'border-gray-100'}`}>
                                    ...
                                    <div className="flex-1 flex flex-col min-w-0">
                                        <div className="p-3 pb-1 flex-1">
                                            ...
                                            <h3 className="font-bold text-sm text-gray-900 leading-tight line-clamp-2 mb-1">{item.title}</h3>
                                            <p className="text-xs text-gray-500 truncate flex items-center gap-1">📍 {item.location || item.location_name || "ไม่ระบุ"}</p>
                                             <p className="text-xs text-gray-500 truncate flex items-center gap-1 mt-0.5">📅 {item.date_display || item.date}</p>
                                        </div>
                                        <div className="flex items-center h-10 border-t border-gray-100 mt-auto">
                                            <button onClick={(e) => handleNavigateToMap(e, item)} className="flex-1 flex items-center justify-center gap-1.5 h-full text-[11px] font-bold text-gray-600 hover:bg-gray-50 transition active:bg-gray-100">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="3 11 22 2 13 21 11 13 3 11"></polygon></svg> นำทาง
                                            </button>
                                            <div className="w-[1px] h-full bg-gray-100"></div>
                                            <button onClick={(e) => handleViewDetails(e, item.id)} className="flex-1 h-full text-[11px] font-bold text-[#FF6B00] hover:bg-orange-50 transition active:bg-orange-100">ดูรายละเอียด</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
