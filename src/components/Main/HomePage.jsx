...
} from "../icons/Icons"; // 👈 ถอย 1 ขั้นเพื่อหา icons
import {
...
export const HomePage = () => {
  ...
  useEffect(() => {
    ...
    if (eventFilter !== "ทั้งหมด") result = result.filter((event) => event.category === eventFilter);
    const now = new Date();
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 pb-16">
      ...
      <section id="news-section" className="mt-8 scroll-mt-28">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {["ทั้งหมด", "K-pop", "T-pop"].map((filter) => (<button key={filter} onClick={() => setHomeNewsFilter(filter)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${homeNewsFilter === filter ? "bg-[#FF6B00] text-white" : "bg-white border text-gray-600 hover:bg-gray-50"}`}>{filter}</button>))}
        </div>
      ...
      <section id="events-section" className="scroll-mt-28">
        ...
        <ScrollableRow className="gap-4 pb-4 -mx-4 px-4 scroll-pl-4">
          {isLoading ? [...Array(5)].map((_, i) => <SkeletonEvent key={i} />) : filteredHomeEvents.length > 0 ? filteredHomeEvents.map((event) => (<div key={event.id} className="flex-shrink-0 w-[38vw] min-w-[140px] md:w-[220px] lg:w-[260px] snap-start h-full"><EventCard item={event} onClick={() => navigate(`/event/${event.id}`, { state: { fromHome: true } })} showNewBadge={eventSort === "newest"} /></div>)) : <EmptyState title="ไม่พบกิจกรรม" subtitle="ลองปรับตัวกรองดูนะ" />}
        </ScrollableRow>
      </section>
      ...
      <div id="cafes-section" className="max-w-6xl mx-auto px-4 scroll-mt-28">
        <div className="flex justify-between items-center mb-6">
          <div className="border-l-4 border-[#5607ff] pl-4"><h2 className="text-2xl font-bold text-gray-900">แนะนำที่จัด Fancafe</h2></div>
